...
display_send_entire_screen_buffer(uint8_t * buffer){
  int i;
  for (i = 0; i < 512; i++){
    display_send_byte(*buffer); // Send each byte in the buffer to the display memory
  buffer++;
  }
}

// Reference: https://digilent.com/reference/_media/chipkit_shield_basic_io_shield:chipkit_basic_io_shield_rm.pdf
void set_entire_display(const uint8_t *buffer)
{
