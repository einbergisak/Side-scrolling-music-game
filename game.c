...
void timer_init() {
  ...
  IECSET(0) = 0x1000;     // Enable interupts for Timer 3
}
...
void update_game() {
  ...
  if (current_level_screen->current_scroll_amount <=
      current_level_screen->entire_image_width - 128) {
    if (!btn_dwn_flag) {
    ...
    if (move_object(current_level_screen, &player)) { // if game is over
      btn_dwn_flag = 1;
      return;
    }
    ...
    current_score = (current_level_screen->current_scroll_amount * 100) /
                    (current_level_screen->entire_image_width - 128);
  } else {
    refresh_screen(&game_over_screen);
